...
for l=1:length(filenames)
    ...
    if findstr(filename, 'C8')
        nodes_outputs = str2num(dataString);
    else
        endIndicationString='Field';
        pEnd = findstr(dataString, endIndicationString);
        dataString = dataString(1:(pEnd-1));
        nodes_outputs1 = str2num(dataString);
        dataString = fileread(sourceFileName2);
        beginIndicationString = '@Loc 1'; % change
        pBegin = strfind(dataString, beginIndicationString);
        dataString = dataString((pBegin(42)+length(beginIndicationString)+374:end));
        nodes_outputs2 = str2num(dataString);
        for i=1:size(nodes_outputs1,1)
            for j=1:size(nodes_outputs2,1)
                if nodes_outputs1(i,1)==nodes_outputs2(j,1)
                    nodes_outputs1(i,2:end)=(nodes_outputs1(i,2:end)+nodes_outputs2(j,2:end))/2;
                    nodes_outputs2(j,:)=NaN;
                end
            end
        end
        nodes_outputs2(isnan(nodes_outputs2(:,1)),:)=[];
        nodes_outputs=[nodes_outputs1; nodes_outputs2];
    end
    %%% Model oparameters
