...
for i = 1 : ll
    for j = 1 : lr
        ...
        % adjust cell coordinates (to apply cell symmetry)
        ...
        % Get one-sided elements (if there are any)
        oc = ECMcirc(El.new, Nodes, config.params.R);  % ic just gives node numbers
        t_handle_elements = toc/60;
        
        tic;
        icNsets = GenNSets(oc); % putting the nodes above into a file
        t_generate_nodesSets = toc/60;
