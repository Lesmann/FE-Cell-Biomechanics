function [ newEl, newNodes, rNodes, RNodes ] = RemoveNodes2 ( N, El, config )
...
[ nodes2remove, newEl ] = RemoveEl(El, rNodes); % Delete elements that include nodes from 'rNodes'

% if strcmp(config.terms.Cells, 'yes') % applied only to models with cells (i.e. not bulk control experiment)
%     N(nodes2remove, :) = [];
% end
