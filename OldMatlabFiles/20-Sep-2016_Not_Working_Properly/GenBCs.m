...
if ~strcmp(config.modelType, 'BCE')
    ...
    for i = 1 : l % going through all nodes on the cell's edge in order to assign them with a BC
        ...
        if i == 1
            ...
            temp = strcat(temp, num2str(-coor(2)*config.params.MOD)); % x BC
            temp = strcat(temp, ' \n');
            temp = strcat(temp, 'ns');
            temp = strcat(temp, num2str(ic(i)));
            temp = strcat(temp, ', 2, 2, ');
            temp = strcat(temp, num2str(-coor(3)*config.params.MOD)); % y BC
            temp = strcat(temp, ' \n');
            bcNames = strcat(bcNames, ', ');
            BCs = strcat(BCs, temp);
        end
    ...
    for i = 1 : l
        ...
        if i == 1
            ...
            temp = strcat(temp, num2str(ic(i)));
            temp = strcat(temp, ', 1, 1, ');
            temp = strcat(temp, num2str(0)); % x BC
            temp = strcat(temp, ' \n');
            temp = strcat(temp, 'ns');
            temp = strcat(temp, num2str(ic(i)));
            temp = strcat(temp, ', 2, 2, ');
    ...
    % Write BCs to txt file
    fn = 'E:\Ran\Cell-ECM_model_2D_1_cell\csvFiles\BCEs.txt';
    fid = fopen(fn, 'wt');
    fprintf(fid, BCs);
