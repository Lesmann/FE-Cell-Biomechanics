...
for i = 1 : l % Going through all elements
    currEl = El(i, 2:3); % Get current element
    nnode1 = currEl(1); nnode2 = currEl(2); % extract element nodes
    if ~ismember(nnode1, Nodes(:, 1)) || ~ismember(nnode2, Nodes(:, 1))
        continue;
    else
        node1 = Nodes(nnode1, :); % Get current element's 1st node's coordinates
        x1 = node1(2); y1 = node1(3); r1 = sqrt(x1^2 + y1^2); % Calculate distance of 1st node from origin
        node2 = Nodes(nnode2, :); % Get current element's 2nd node's coordinates
        x2 = node2(2); y2 = node2(3); r2 = sqrt(x2^2 + y2^2); % Calculate distance of 2nd node from origin
    end
...
dlmwrite(fn, WeakEl, 'precision', '%1.0f');
...
fn = strcat(path, fileName);
dlmwrite(fn, NormEl, 'precision', '%1.0f');
