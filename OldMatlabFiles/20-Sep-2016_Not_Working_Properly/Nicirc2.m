function [ inCirc ] = Nicirc2 ( Elements, Nodes, r, cellCoordinates, config )
...
for j = 1 : length(cellCoordinates) % going through all cells
    for i = 1 : length(N) % going through all nodes
        ...
        if d < r+3*iSeed % if the current node is connected to 5 or 6 elements, and it is quite near the cell centre
