% created by Hanan Tokash; adapted by Dr. Ran Sopher, 18-Jul-2016
...
% ** inp model file should be present in the destination folder 'fn' **
...
for i = 1 : ll
    for j = 1 : lr
        ...
        display(['generating model ', num2str(j), '-', num2str(i)]);
        ...
        if ~strcmp(config.modelType, 'BCE')
            [ El, Nodes, rNodes, oc ] = RemoveNodes2(Nodes, El, config);
        end
        
        % Remove extra nodes and elements
        % Nodes(rNodes, :) = [];
        ...
        if ~isempty(config.cells) % a single cell
            config.cells = AdjustMCC(config, Nodes);
        end
        ...
        if ~strcmp(config.modelType, 'BCE')
            ...
            for k = 1 : nTotal % Go over all nodes
                x = Nodes(k, 1); % Get current node coordinates
                y = Nodes(k, 2);
                [ newx, newy ] = cirrdn(x, y, config.ROR); % Generate coordinates of new node inside the circle
                Nodes(k, 1) = newx; % Initiate coordinates to nodes vector
                Nodes(k, 2) = newy;
            end
        end
...
%         end
        ...
        FN = GenFileName(config.ROR(j)/config.params.iSeed, config.LOC(i), config.terms.Cells, config.cells, config.modelType, config.blMatProp.type);
        [ mat ] = defmat(config, FN);
        
        %% Generate node-sets from inner nodes
        
        display('Generating Node Sets')
        ...
        if isempty(config.cells) % i.e., a model of a single cell
            ...
            ic = Nicirc2(El.new, Nodes, config.params.r, config.cells, config); % ic just gives both node numbers (column 2) and cell numbers (column 1)
        end
        
        t_handle_elements = toc/60;
        hold on, scatter(Nodes(:, 2), Nodes(:, 3), '.'); scatter(Nodes(ic(:, 2), 2), Nodes(ic(:, 2), 3), '.');
        
        tic;
        if isempty(config.cells) % a single cell
            icNsets = GenNSets(ic); % putting the nodes above into a file
        else
            icNsets = GenNSets(ic(:, 2)); % putting the nodes above into a file
        end
        ...
        if strcmp(config.terms.Cells, 'yes')
            if strcmp(config.terms.Contraction,'U') % if the BCs/LCs of the model are BCs
                ...
                %% HERE %%%
                % if ~strcmp(config.modelType, 'BCE')
                    % [ fixationBCs, fixationbcNames ] =
                    % GenfixationBCs(Nodes);
                % end
                %% HERE %%%
                if isempty(config.cells) % a single cell or BCE
                    if strcmp(config.modelType, 'BCE')
                        edges = findEdge(El.new, Nodes, config);
                        [ BCs, bcNames ] = GenBCs(edges, Nodes, config);
                    else
...
%tt = num2str(sum(T));
path_config = 'E:\Ran\Cell-ECM_model_2D_1_cell\LastConfig\config.mat';
save(path_config, '-struct', 'config');
