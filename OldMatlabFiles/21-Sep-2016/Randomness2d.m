...
for i = 1 : nTotal % Go over all nodes
    ...
    if ~isempty(config.cells)
        ...
        if ~strcmp(config.modelType, 'BCE')
            if D < config.params.R-config.params.iSeed && D > config.params.r+2*config.params.iSeed % The following statements do not apply to nodes adjacent to the edges of the ECM and cell
                [ newx, newy ] = cirrdn(x, y, ROR); % Generate coordinates of new node inside the circle
                nodes(i, 1) = newx; % Initiate coordinates to nodes vector
                nodes(i, 2) = newy;
            end
