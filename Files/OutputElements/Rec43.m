function out = Rec43(Rec)
%
% ABAQUS total fluid volume ratio output to MATLAB
% 
% Syntax
%     #Rec# = Fil2str('*.fil');
%     #out# = Rec43(#Rec#)
%
% Description
%     Read total fluid volume ratio output from the results (*.fil) file
...
%     fluid volume ratio output is 43 (only in ABAQUS/Standard). See
...
%     contain total fluid volume ratio results:
%         ...
%         *FILE FORMAT, ASCII
%         *EL FILE
%         FLUVR
...
%         the record key 43 as follows:
%         Column  1  –  Total fluid volume ratio.
...
ind = strfind(Rec,'I 243D'); % record key for element total fluid volume ratio output (43)
if isempty(ind)
    out=[];
    return;
end
nextpos=numel('I 243')+1;
for i=1:numel(ind)
    % check ind
...
for i=1:numel(ind)
    ...
    % Element total fluid volume ratio
