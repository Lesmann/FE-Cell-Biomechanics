function out = Rec402(Rec)
%
% ABAQUS principal values of backstress tensor for kinematic hardening
% plasticity output to MATLAB
% 
% Syntax
%     #Rec# = Fil2str('*.fil');
%     #out# = Rec402(#Rec#)
%
% Description
%     Read principal values of backstress tensor for kinematic hardening
%     plasticity output from the results (*.fil) file generated from the
%     ABAQUS finite element software. The asterisk (*) is replaced by the
%     name of the results file. The record key for principal values of
%     backstress tensor for kinematic hardening plasticity output is 402.
...
%     contain principal values of backstress tensor for kinematic hardening
%     plasticity results:
%         ...
%         *FILE FORMAT, ASCII
%         *EL FILE
...
%         the record key 402 as follows:
%         Column  1  -  Minimum principal value of backstress tensor for
...
ind = strfind(Rec,'I 3402D'); % record key for element principal values of backstress tensor for kinematic hardening plasticity output (402)
if isempty(ind)
    out=[];
    return;
end
nextpos=numel('I 3402')+1;
...
for i=1:numel(ind)
    ...
    % Element principal values of backstress tensor for kinematic hardening
    % plasticity
    for j=1:NW(i)-2
        % principal value of backstress tensor for kinematic hardening
        % plasticity
