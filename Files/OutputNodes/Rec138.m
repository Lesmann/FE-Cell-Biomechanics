function out = Rec138(Rec)
%
% ABAQUS electrical reaction current output to MATLAB
% 
% Syntax
%     #Rec# = Fil2str('*.fil');
%     #out# = Rec138(#Rec#)
%
% Description
%     Read electrical reaction current output from the results (*.fil) file
...
%     electrical reaction current output is 138. See section < < Results file
...
%     contain electrical reaction current results:
%         ...
%         *FILE FORMAT, ASCII
%         *NODE FILE
%         RECUR
...
ind = strfind(Rec,'I 3138I'); % record key for node electrical reaction current output (138)
if isempty(ind)
    out=[];
    return;
end
nextpos=numel('I 3138')+1;
for i=1:numel(ind)
    % check the record
