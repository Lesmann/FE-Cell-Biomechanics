...
filenames_2cells={filenames_2cells{1},filenames_2cells{3},filenames_2cells{2},filenames_2cells{4}};

for l=1:length(filenames_2cells)
    clearvars -except filenames_2cells filename_1cell R l anglefraction_maxprincipalstrain anglefraction_minprincipalstrain R_maxprincipalstrain_ratio R_minprincipalstrain_ratio R_maxprincipalstrain_2cells_normalisedR R_minprincipalstrain_2cells_normalisedR anglefraction_maxprincipalstrain_ratio anglefraction_minprincipalstrain_ratio R_xx_maxprincipalstrain_ratio R_yy_maxprincipalstrain_ratio R_xy_maxprincipalstrain_ratio R_xx_minprincipalstrain_ratio R_yy_minprincipalstrain_ratio R_xy_minprincipalstrain_ratio R_xx_maxprincipalstrain_2cells_normalisedR R_yy_maxprincipalstrain_2cells_normalisedR R_xy_maxprincipalstrain_2cells_normalisedR R_xx_minprincipalstrain_2cells_normalisedR R_yy_minprincipalstrain_2cells_normalisedR R_xy_minprincipalstrain_2cells_normalisedR dfo_list
...
%     axis equal
    ...
    centroids_ROI_shifted(:,2)=centroids_ROI_shifted(:,2)-config.cells(1,1);
    centroids_ROI_shifted(:,3)=centroids_ROI_shifted(:,3)-config.cells(1,2);
    % M0
    ...
    hold on
    plot(centroids_ROI_shifted(:,2),centroids_ROI_shifted(:,3),'.','MarkerSize',16);
    plot(R_x_maxprincipalstrain_2cells(l),R_y_maxprincipalstrain_2cells(l),'rX','MarkerSize',16);
    plot(R_x_minprincipalstrain_2cells(l),R_y_minprincipalstrain_2cells(l),'r+','MarkerSize',16);
    plot(R_xx_maxprincipalstrain_2cells(l),R_yy_maxprincipalstrain_2cells(l),'gX','MarkerSize',16);
    plot(R_xx_minprincipalstrain_2cells(l),R_yy_minprincipalstrain_2cells(l),'g+','MarkerSize',16);
    title(strcat('First and second moments - 2 cells (distance: ',num2str(dfo),') (BL, TFBC)'));
    legend('','First moment of max principal strain','First moment of min principal strain','Second moment of max principal strain','Second moment of min principal strain');
    axis equal
...
%     axis equal
    ...
    clearvars centroids_outputs2 centroids_outputs centroids_data current_cellcentre current_centroid M offset_centroids_data offset_centroids_data_1cell centroids_ROI centroids_angle
...
%     axis equal
    ...
    firstmoment_y_minprincipalstrain_1cell(l)=(centroids_ROI(:,3)'*centroids_ROI(:,5));
    ...
    hold on
    plot(centroids_ROI(:,2),centroids_ROI(:,3),'.','MarkerSize',16);
    plot(R_x_maxprincipalstrain_1cell(l),R_y_maxprincipalstrain_1cell(l),'rX','MarkerSize',16);
    plot(R_x_minprincipalstrain_1cell(l),R_y_minprincipalstrain_1cell(l),'r+','MarkerSize',16);
    plot(R_xx_maxprincipalstrain_1cell(l),R_yy_maxprincipalstrain_1cell(l),'gX','MarkerSize',16);
    plot(R_xx_minprincipalstrain_1cell(l),R_yy_minprincipalstrain_1cell(l),'g+','MarkerSize',16);
    title(strcat('First and second moments - 1 cell (distance: ',num2str(dfo),') (BL, TFBC)'));
    legend('','First moment of max principal strain','First moment of min principal strain','Second moment of max principal strain','Second moment of min principal strain');
    axis equal
    set(gcf,'units','normalized','outerposition',[0 0 1 1])
    
    %% Calculating ratios
    anglefraction_maxprincipalstrain_ratio(l)=anglefraction_maxprincipalstrain_2cells(l)/anglefraction_maxprincipalstrain_1cell(l);
    anglefraction_minprincipalstrain_ratio(l)=anglefraction_minprincipalstrain_2cells(l)/anglefraction_minprincipalstrain_1cell(l);
    R_maxprincipalstrain_ratio(l)=R_maxprincipalstrain_2cells(l)/R_maxprincipalstrain_1cell(l);
    R_minprincipalstrain_ratio(l)=R_minprincipalstrain_2cells(l)/R_minprincipalstrain_1cell(l);
    R_maxprincipalstrain_2cells_normalisedR(l)=R_maxprincipalstrain_2cells(l)/cellRadius;
    R_minprincipalstrain_2cells_normalisedR(l)=R_minprincipalstrain_2cells(l)/cellRadius;
    R_xx_maxprincipalstrain_ratio(l)=R_xx_maxprincipalstrain_2cells(l)/R_xx_maxprincipalstrain_1cell(l);
    R_yy_maxprincipalstrain_ratio(l)=R_yy_maxprincipalstrain_2cells(l)/R_yy_maxprincipalstrain_1cell(l);
    R_xy_maxprincipalstrain_ratio(l)=R_xy_maxprincipalstrain_2cells(l)/R_xy_maxprincipalstrain_1cell(l);
    R_xx_minprincipalstrain_ratio(l)=R_xx_minprincipalstrain_2cells(l)/R_xx_minprincipalstrain_1cell(l);
    R_yy_minprincipalstrain_ratio(l)=R_yy_minprincipalstrain_2cells(l)/R_yy_minprincipalstrain_1cell(l);
    R_xy_minprincipalstrain_ratio(l)=R_xy_minprincipalstrain_2cells(l)/R_xy_minprincipalstrain_1cell(l);
    R_xx_maxprincipalstrain_2cells_normalisedR(l)=R_xx_maxprincipalstrain_2cells(l)/cellRadius;
    R_yy_maxprincipalstrain_2cells_normalisedR(l)=R_yy_maxprincipalstrain_2cells(l)/cellRadius;
    R_xy_maxprincipalstrain_2cells_normalisedR(l)=R_xy_maxprincipalstrain_2cells(l)/cellRadius;
    R_xx_minprincipalstrain_2cells_normalisedR(l)=R_xx_minprincipalstrain_2cells(l)/cellRadius;
    R_yy_minprincipalstrain_2cells_normalisedR(l)=R_yy_minprincipalstrain_2cells(l)/cellRadius;
    R_xy_minprincipalstrain_2cells_normalisedR(l)=R_xy_minprincipalstrain_2cells(l)/cellRadius;

end

xlabels=dfo_list;
figure
bar(R_maxprincipalstrain_ratio)
title('First moment for max principal strain (normalised according to the equivalent single-cell model) (BL, TFBC)')
set(gca,'XTickLabel',xlabels,'XTick',1:numel(xlabels))
set(gcf,'units','normalized','outerposition',[0 0 1 1])
figure
bar(R_minprincipalstrain_ratio)
title('First moment for min principal strain (normalised according to the equivalent single-cell model) (BL, TFBC)')
set(gca,'XTickLabel',xlabels,'XTick',1:numel(xlabels))
set(gcf,'units','normalized','outerposition',[0 0 1 1])
figure
bar(R_maxprincipalstrain_2cells_normalisedR);
title('First moment for max principal strain (normalised according to a cell radius) (BL, TFBC)')
set(gca,'XTickLabel',xlabels,'XTick',1:numel(xlabels))
set(gcf,'units','normalized','outerposition',[0 0 1 1])
figure
bar(R_minprincipalstrain_2cells_normalisedR);
title('First moment for min principal strain (normalised according to a cell radius) (BL, TFBC)')
set(gca,'XTickLabel',xlabels,'XTick',1:numel(xlabels))
set(gcf,'units','normalized','outerposition',[0 0 1 1])
figure
bar(R_xx_maxprincipalstrain_ratio)
title('Second moment (xx) for max principal strain (normalised according to the equivalent single-cell model) (BL, TFBC)')
set(gca,'XTickLabel',xlabels,'XTick',1:numel(xlabels))
set(gcf,'units','normalized','outerposition',[0 0 1 1])
figure
bar(R_yy_maxprincipalstrain_ratio)
title('Second moment (yy) for max principal strain (normalised according to the equivalent single-cell model) (BL, TFBC)')
set(gca,'XTickLabel',xlabels,'XTick',1:numel(xlabels))
set(gcf,'units','normalized','outerposition',[0 0 1 1])
% figure
% bar(R_xy_maxprincipalstrain_ratio)
% title('Second moment (xy) for max principal strain (normalised according to the equivalent single-cell model) (BL, TFBC)')
% set(gca,'XTickLabel',xlabels,'XTick',1:numel(xlabels))
% set(gcf,'units','normalized','outerposition',[0 0 1 1])
figure
bar(R_xx_minprincipalstrain_ratio)
title('Second moment (xx) for min principal strain (normalised according to the equivalent single-cell model) (BL, TFBC)')
set(gca,'XTickLabel',xlabels,'XTick',1:numel(xlabels))
set(gcf,'units','normalized','outerposition',[0 0 1 1])
figure
bar(R_yy_minprincipalstrain_ratio)
title('Second moment (yy) for min principal strain (normalised according to the equivalent single-cell model) (BL, TFBC)')
set(gca,'XTickLabel',xlabels,'XTick',1:numel(xlabels))
set(gcf,'units','normalized','outerposition',[0 0 1 1])
% figure
% bar(R_xy_minprincipalstrain_ratio)
% title('Second moment (xy) for min principal strain (normalised according to the equivalent single-cell model) (BL, TFBC)')
% set(gca,'XTickLabel',xlabels,'XTick',1:numel(xlabels))
% set(gcf,'units','normalized','outerposition',[0 0 1 1])
figure
bar(R_xx_maxprincipalstrain_2cells_normalisedR);
title('Second moment (xx) for max principal strain (normalised according to a cell radius) (BL, TFBC)')
set(gca,'XTickLabel',xlabels,'XTick',1:numel(xlabels))
set(gcf,'units','normalized','outerposition',[0 0 1 1])
figure
bar(R_yy_maxprincipalstrain_2cells_normalisedR);
title('Second moment (yy) for max principal strain (normalised according to a cell radius) (BL, TFBC)')
set(gca,'XTickLabel',xlabels,'XTick',1:numel(xlabels))
set(gcf,'units','normalized','outerposition',[0 0 1 1])
% figure
% bar(R_xy_maxprincipalstrain_2cells_normalisedR);
% title('Second moment (xy) for max principal strain (normalised according to a cell radius) (BL, TFBC)')
% set(gca,'XTickLabel',xlabels,'XTick',1:numel(xlabels))
% set(gcf,'units','normalized','outerposition',[0 0 1 1])
figure
bar(R_xx_minprincipalstrain_2cells_normalisedR);
title('Second moment (xx) for min principal strain (normalised according to a cell radius) (BL, TFBC)')
set(gca,'XTickLabel',xlabels,'XTick',1:numel(xlabels))
set(gcf,'units','normalized','outerposition',[0 0 1 1])
figure
bar(R_yy_minprincipalstrain_2cells_normalisedR);
title('Second moment (yy) for min principal strain (normalised according to a cell radius) (BL, TFBC)')
set(gca,'XTickLabel',xlabels,'XTick',1:numel(xlabels))
set(gcf,'units','normalized','outerposition',[0 0 1 1])
% figure
% bar(R_xy_minprincipalstrain_2cells_normalisedR);
% title('Second moment (xy) for min principal strain (normalised according to a cell radius) (BL, TFBC)')
% set(gca,'XTickLabel',xlabels,'XTick',1:numel(xlabels))
% set(gcf,'units','normalized','outerposition',[0 0 1 1])
figure
bar(anglefraction_maxprincipalstrain_ratio);
title('Fraction of max principal strain falling within a 60-degrees sector (normalised according to the equivalent single-cell model) (BL, TFBC)')
set(gca,'XTickLabel',xlabels,'XTick',1:numel(xlabels))
set(gcf,'units','normalized','outerposition',[0 0 1 1])
figure
bar(anglefraction_minprincipalstrain_ratio);
title('Fraction of min principal strain falling within a 60-degrees sector (normalised according to the equivalent single-cell model) (BL, TFBC)')
set(gca,'XTickLabel',xlabels,'XTick',1:numel(xlabels))
set(gcf,'units','normalized','outerposition',[0 0 1 1])

h=findobj('type','figure');
numoffigures=length(h);

for i=1:numoffigures
    saveas(figure(i),strcat(num2str(i),'.jpg'));
end    
